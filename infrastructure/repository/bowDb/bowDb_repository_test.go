...
var _ = Describe("*bowDb.ScanItemRepository", func() {

	Describe(":: GetRepoName() to get Repository's name", func() {
		repoName := "testRepo"
		repo, _ := setupDb().InitRepository(repoName)
		defer repo.CloseDb()
	...
	Describe(":: NewItem(Key, Data) to save an Item", func() {
		Context(" :GIVEN: an item-key was EMPTY/BLANK", func() {
			repoName := "testRepo"
			sut, _ := setupDb().InitRepository(repoName)
			defer sut.CloseDb()

			Context(" :THEN ⇶ call to SetItem(item)", func() {
				var inputKey string = ""
				var inputData = map[string]string{"field1": "data1", "field2": "data2"}
				var got, err = sut.NewItem(inputKey, inputData)
